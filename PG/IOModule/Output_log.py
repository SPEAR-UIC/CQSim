...
class Output_log:
    ...
    def reset_output(self):   
        ...
        self.reward_info = Log_print.Log_print(self.output_path['reward'],0)
        self.reward_info.reset(self.output_path['reward'],0)
        self.reward_info.file_open()
        self.reward_info.file_close()
        self.reward_info.reset(self.output_path['reward'],1)     

    def print_reward_info(self, reward_info=None):     
        if reward_info != None:
            self.reward_buf.append(reward_info)
        if (len(self.reward_buf) >= self.log_freq) or (reward_info == None):
            #pre_context = "Printing..............................\n"
            self.reward_info.file_open()
            for reward in self.reward_buf:
                #context = pre_context+""
                #pre_context = ""
                context = str(reward)
                self.reward_info.log_print(context,1)
            self.reward_info.file_close()
            self.reward_buf = []
    ...
    def print_sys_info(self, sys_info = None):
        ...
        if (len(self.sys_info_buf) >= self.log_freq) or (sys_info == None):
            ...
            for sys_info in self.sys_info_buf:
                ...
                context += ('waitSize'+'='+str(sys_info['waitSize']))
                context += sep_sign_B
                context += ('waitCore'+'='+str(sys_info['waitCore']))
                self.sys_info.log_print(context,1)
            self.sys_info.file_close()
            self.sys_info_buf = []
