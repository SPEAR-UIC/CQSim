...
double fcfs_plan::planfirst(idproc pj[])
{
	...
	temptime /= totaljob; // this is AWRT
	...
	temptime2 /= totaljob;// this is the mean squared wait time This is SW
	
	return temptime2;    // This is the mean squared wait time SW
...
void fcfs_plan::random()
{
	//Simple version everything start here
	//1,find base solution
	//2.find neighbor solution 
	//3.check neighbor solution
	//4.if better, update. if not, continue
