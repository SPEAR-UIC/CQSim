...
import CqSim.BurstBuffer_struc as Class_BurstBuffer_struc
...
def  cqsim_main(para_list):
    ...
    output_distance = para_list['path_out'] + para_list['output'] + para_list['distance_file']
    util_file = para_list['path_out'] + para_list['output'] + para_list['util']
    time_file = para_list['path_out'] + para_list['output'] + para_list['time_file']
    output_fn = {'sys':output_sys, 'adapt':output_adapt, 'result':output_result, 'distance': output_distance, 'util': util_file, 'time_file': time_file}
    ...
    module_debug = Class_Debug_log.Debug_log(lvl=para_list['debug_lvl'],show=2,path=debug_path)
    ...
    # Job Trace
    print ".................... Job Trace"
    module_job_trace = Class_Job_trace.Job_trace(start=para_list['start'],num=para_list['read_num'],anchor=para_list['anchor'],density=para_list['cluster_fraction'],debug=module_debug)
    module_job_trace.import_job_file(save_name_j, module_filter_node)
    ...
    #Class_BurstBuffer_struc
    print ".................... BurstBuffer Structure"
    module_bb_struc = Class_BurstBuffer_struc.BurstBuffer_struc(debug=module_debug)
    module_bb_struc.import_bb_config(config_name_n)
    ...
    # Output Log
    print ".................... Output Log"
    module_output_log = Class_Output_log.Output_log (output=output_fn)
    
    # Cqsim Simulator
    print ".................... Cqsim Simulator"
    module_list = {'job':module_job_trace,'node':module_node_struc,'backfill':module_backfill,'bb':module_bb_struc,\
                   'win':module_win,'alg':module_alg,'info':module_info_collect, 'output':module_output_log}
    module_sim = Class_Cqsim_sim.Cqsim_sim (module=module_list, debug=module_debug, monitor = para_list['monitor'], schedule = para_list['schedule'], win_size = para_list['win_size'], generations = para_list['generations'])
